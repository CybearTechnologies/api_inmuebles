...
DELIMITER $$
CREATE PROCEDURE insertAgency(name varchar(45), icon varchar(255),user int, dateCreated datetime)
BEGIN
    IF IsNull(dateCreated) THEN
        INSERT INTO agency(ag_name, ag_icon,ag_user_created_fk, ag_user_modified_fk)
        VALUES (name, icon,user, user);
    ELSE
        INSERT INTO agency(ag_name,ag_icon, ag_user_created_fk, ag_user_modified_fk, ag_date_created, ag_date_modified)
...
DELIMITER $$
CREATE PROCEDURE updateAgency(id int, icon varchar(255),name varchar(45), user int, dateModified datetime)
BEGIN
